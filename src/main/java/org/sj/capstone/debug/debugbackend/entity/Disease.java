package org.sj.capstone.debug.debugbackend.entity;

import javax.persistence.Column;
import javax.persistence.Embeddable;
import javax.persistence.Entity;
import java.util.HashMap;
import java.util.Map;

@Embeddable
public class Disease {

    @Column(name = "disease1", nullable = false)
    private double disease1;

    @Column(name = "disease2", nullable = false)
    private double disease2;

    @Column(name = "disease3", nullable = false)
    private double disease3;

    @Column(name = "disease4", nullable = false)
    private double disease4;

    @Column(name = "disease5", nullable = false)
    private double disease5;

    @Column(name = "disease6", nullable = false)
    private double disease6;

    @Column(name = "disease7", nullable = false)
    private double disease7;

    @Column(name = "disease8", nullable = false)
    private double disease8;

    @Column(name = "disease9", nullable = false)
    private double disease9;

    @Column(name = "disease10", nullable = false)
    private double disease10;

    @Column(name = "disease11", nullable = false)
    private double disease11;

    @Column(name = "disease12", nullable = false)
    private double disease12;

    @Column(name = "disease13", nullable = false)
    private double disease13;

    @Column(name = "disease14", nullable = false)
    private double disease14;

    @Column(name = "disease15", nullable = false)
    private double disease15;

    @Column(name = "disease16", nullable = false)
    private double disease16;

    @Column(name = "disease17", nullable = false)
    private double disease17;

    @Column(name = "disease18", nullable = false)
    private double disease18;

    @Column(name = "disease19", nullable = false)
    private double disease19;

    @Column(name = "disease20", nullable = false)
    private double disease20;

    @Column(name = "disease21", nullable = false)
    private double disease21;

    @Column(name = "disease22", nullable = false)
    private double disease22;

    @Column(name = "disease23", nullable = false)
    private double disease23;

    @Column(name = "disease24", nullable = false)
    private double disease24;

    @Column(name = "disease25", nullable = false)
    private double disease25;

    @Column(name = "disease26", nullable = false)
    private double disease26;

    @Column(name = "disease27", nullable = false)
    private double disease27;

    @Column(name = "disease28", nullable = false)
    private double disease28;

    @Column(name = "disease29", nullable = false)
    private double disease29;

    @Column(name = "disease30", nullable = false)
    private double disease30;

    @Column(name = "disease31", nullable = false)
    private double disease31;

    @Column(name = "disease32", nullable = false)
    private double disease32;

    @Column(name = "disease33", nullable = false)
    private double disease33;

    @Column(name = "disease34", nullable = false)
    private double disease34;

    @Column(name = "disease35", nullable = false)
    private double disease35;

    @Column(name = "disease36", nullable = false)
    private double disease36;

    @Column(name = "disease37", nullable = false)
    private double disease37;

    @Column(name = "disease38", nullable = false)
    private double disease38;

    @Column(name = "disease39", nullable = false)
    private double disease39;

    @Column(name = "disease40", nullable = false)
    private double disease40;

    @Column(name = "disease41", nullable = false)
    private double disease41;

    @Column(name = "disease42", nullable = false)
    private double disease42;

    @Column(name = "disease43", nullable = false)
    private double disease43;

    @Column(name = "disease44", nullable = false)
    private double disease44;

    @Column(name = "disease45", nullable = false)
    private double disease45;

    @Column(name = "disease46", nullable = false)
    private double disease46;

    @Column(name = "disease47", nullable = false)
    private double disease47;

    @Column(name = "disease48", nullable = false)
    private double disease48;

    @Column(name = "disease49", nullable = false)
    private double disease49;

    @Column(name = "disease50", nullable = false)
    private double disease50;

    @Column(name = "disease51", nullable = false)
    private double disease51;

    @Column(name = "disease52", nullable = false)
    private double disease52;

    @Column(name = "disease53", nullable = false)
    private double disease53;

    @Column(name = "disease54", nullable = false)
    private double disease54;

    @Column(name = "disease55", nullable = false)
    private double disease55;

    @Column(name = "disease56", nullable = false)
    private double disease56;

    @Column(name = "disease57", nullable = false)
    private double disease57;

    @Column(name = "disease58", nullable = false)
    private double disease58;

    @Column(name = "disease59", nullable = false)
    private double disease59;

    @Column(name = "disease60", nullable = false)
    private double disease60;

    @Column(name = "disease61", nullable = false)
    private double disease61;

    @Column(name = "disease62", nullable = false)
    private double disease62;

    @Column(name = "disease63", nullable = false)
    private double disease63;

    @Column(name = "disease64", nullable = false)
    private double disease64;

    @Column(name = "disease65", nullable = false)
    private double disease65;

    @Column(name = "disease66", nullable = false)
    private double disease66;

    @Column(name = "disease67", nullable = false)
    private double disease67;

    @Column(name = "disease68", nullable = false)
    private double disease68;

    @Column(name = "disease69", nullable = false)
    private double disease69;

    @Column(name = "disease70", nullable = false)
    private double disease70;

    @Column(name = "disease71", nullable = false)
    private double disease71;

    @Column(name = "disease72", nullable = false)
    private double disease72;

    @Column(name = "disease73", nullable = false)
    private double disease73;

    @Column(name = "disease74", nullable = false)
    private double disease74;

    @Column(name = "disease75", nullable = false)
    private double disease75;

    static public Map<String, String> DISEASE_NAME_MAP = new HashMap<>() {{
        put("disease1", "팥_흰가루병");
        put("disease2", "팥_세균잎마름병");
        put("disease3", "팥_Rhizopus");
        put("disease4", "참깨_세균성점무늬병");
        put("disease5", "참깨_시들음병");
        put("disease6", "참깨_잎마름병");
        put("disease7", "참깨_흰가루병");
        put("disease8", "콩_노균병");
        put("disease9", "콩_들불병");
        put("disease10", "콩_불마름병");
        put("disease11", "콩_병징");
        put("disease12", "가지_잎곰팡이병");
        put("disease13", "가지_흰가루병");
        put("disease14", "고추_마일드모틀바이러스병");
        put("disease15", "고추_점무늬병");
        put("disease16", "단호박_점무늬병");
        put("disease17", "단호박_흰가루병");
        put("disease18", "딸기_잿빛곰팡이병");
        put("disease19", "딸기_흰가루병");
        put("disease20", "상추_균핵병");
        put("disease21", "상추_노균병");
        put("disease22", "수박_탄저병");
        put("disease23", "수박_흰가루병");
        put("disease24", "애호박_점무늬병");
        put("disease25", "오이_녹반모자이크바이러스");
        put("disease26", "쥬키니호박_오이녹반모자이크바이러스");
        put("disease27", "참외_노균병");
        put("disease28", "참외_흰가루병");
        put("disease29", "토마토_잎곰팡이병");
        put("disease30", "토마토_황화잎말이바이러스병");
        put("disease31", "포도_노균병");
        put("disease32", "검거세미밤나방");
        put("disease33", "꽃노랑총채벌레");
        put("disease34", "담배가루이");
        put("disease35", "담배거세미나방");
        put("disease36", "담배나방");
        put("disease37", "도둑나방");
        put("disease38", "먹노린재");
        put("disease39", "목화바둑명나방");
        put("disease40", "무잎벌");
        put("disease41", "배추좀나방");
        put("disease42", "배추흰나비");
        put("disease43", "벼룩잎벌레");
        put("disease44", "복숭아혹진딧물");
        put("disease45", "비단노린재");
        put("disease46", "썩덩나무노린재");
        put("disease47", "열대거세미나방");
        put("disease48", "큰28점박이무당벌레");
        put("disease49", "톱다리개미허리노린재");
        put("disease50", "파밤나방");
        put("disease51", "고추_탄저병");
        put("disease52", "고추_흰가루병");
        put("disease53", "무_검은무늬병");
        put("disease54", "무_노균병");
        put("disease55", "배추_검은썩음병");
        put("disease56", "배추_노균병");
        put("disease57", "애호박_노균병");
        put("disease58", "애호박_흰가루병");
        put("disease59", "양배추_균핵병");
        put("disease60", "양배추_무름병");
        put("disease61", "오이_노균병");
        put("disease62", "오이_흰가루병");
        put("disease63", "콩_점무늬병");
        put("disease64", "토마토_잎마름병");
        put("disease65", "파_검은무늬병");
        put("disease66", "파_노균병");
        put("disease67", "파_녹병");
        put("disease68", "호박_노균병");
        put("disease69", "호박_흰가루병");
        put("disease70", "배과수_화상병");
        put("disease71", "사과_갈색무늬병");
        put("disease72", "사과_과수화상병");
        put("disease73", "사과_부란병");
        put("disease74", "사과_점무늬낙엽병");
        put("disease75", "사과_탄저병");
    }};
}
